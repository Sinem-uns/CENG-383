...
def read_results(filename=None):
    if filename is None:
        filename = Path(__file__).resolve().parents[1] / "results" / "runtime_memory_results.csv"
...
def plot_runtime(nodes, dijkstra_times, bellman_times, output_path=None):
    if output_path is None:
        output_path = Path(__file__).resolve().parents[1] / "results" / "plots" / "runtime_comparison.png"
...
def plot_memory(nodes, dijkstra_memory, bellman_memory, output_path=None):
    if output_path is None:
        output_path = Path(__file__).resolve().parents[1] / "results" / "plots" / "memory_comparison.png"
