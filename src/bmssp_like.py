...
Constraint: Non-negative edge weights only (same as Dijkstra). Negative
            weights would allow a node's distance to decrease after it has
            been settled, causing it to be re-inserted into an already-
            processed bucket -- producing an infinite loop. A ValueError is
            raised if a negative weight is detected.
"""
...
def bmssp_like(graph, source):
    # BUG FIX: Validate that all edge weights are non-negative before starting.
    # Without this check, a negative weight causes a node to be re-added to an
    # already-processed (deleted) bucket, which triggers an infinite loop because
    # the bucket is re-created by defaultdict and never drained.
    for u, neighbors in graph.items():
        for v, w in neighbors:
            if w < 0:
                raise ValueError(
                    f"bmssp_like does not support negative edge weights "
                    f"(edge {u} -> {v} has weight {w}). "
                    f"Use Bellman-Ford for graphs with negative weights."
                )
